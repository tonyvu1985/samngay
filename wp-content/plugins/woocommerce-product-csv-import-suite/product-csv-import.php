		...
		class WC_Product_CSV_Import_Suite {
			...
			public function tools( $tools ) {
				...
				$tools['delete_variations'] = array(
					...
					'desc'		=> __( 'This tool will delete all product allowing you to start fresh.', 'wc_csv_import' ),
			...
			public function admin_import_page() {
				...
				<div id="message" class="updated woocommerce-message wc-connect">
					<div class="squeezer">
						...
						<p class="submit"><a href="http://docs.woothemes.com/documentation/plugins/woocommerce/woocommerce-extensions/product-csv-import-suite/" class="button-primary"><?php _e( 'Documentation', 'wc_csv_import' ); ?></a> <a class="docs button-primary" href="<?php echo plugins_url( 'sample.csv', __FILE__ ); ?>"><?php _e('Sample CSV', 'wc_csv_import'); ?></a></p>
				...
				<div class="tool-box">
					...
					<form action="<?php echo admin_url('admin.php?page=woocommerce_csv_import_suite&action=export_variations'); ?>" method="post">
						...
						<table class="form-table">
							...
							<tr>
								<th>
									<label for="product"><?php _e( 'Limit to parent ID', 'wc_csv_import' ); ?></label>
								</th>
								<td>
									<input type="text" name="product_limit" id="product_limit" placeholder="<?php _e( 'N/A', 'wc_csv_import' ); ?>" class="input-text" />
								</td>
							</tr>
							<tr>
								<th>
			...
			public function product_exporter( $post_type = 'product' ) {
				...
				while ( $export_count < $export_limit ) {
					...
					if ( $post_type == 'product_variation' ) {
						...
						if ( $product_limit ) {
							$product_args['post_parent'] = $product_limit;
						}
					}
