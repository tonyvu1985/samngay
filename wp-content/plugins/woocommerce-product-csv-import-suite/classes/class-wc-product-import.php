...
class WC_CSV_Product_Import extends WP_Importer {
							...
							$('body').on( 'woocommerce_csv_import_request_complete', function() {
								if ( done_count == <?php echo $import_count; ?> ) {
									if ( attachments.length ) {
										...
										regenerate_thumbnail( attachments.shift() );
									} else {
										import_done();
									}
								} else {
							...
							function regenerate_thumbnail( id ) {
								$.ajax({
									...
									success: function( response ) {
										...
										$('#import-progress tbody .regenerating .progress').css( 'width', ( ( regen_count / ( regen_count + attachments.length ) ) * 100 ) + '%' ).html( regen_count + ' / ' + ( regen_count + attachments.length ) + ' <?php echo esc_js( __( 'thumbnails regenerated', 'wc_csv_import' ) ); ?>' );

										if ( response.success ) {
											//$('#import-progress tbody').append( '<tr class="regenerated"><td><mark class="result" title="Success">Success</mark></td><td class="row">' + regen_count + '</td><td></td><td colspan="2">' + response.success + '</td></tr>' );
										} else {
	...
	/**
	 * Generate meta data for attachments
	 * @param  array $attachments
	 */
	public function generate_thumbnails( $attachments ) {
		foreach ( $attachments as $attachment_id => $attachment_file ) {
			$metadata = wp_generate_attachment_metadata( $attachment_id, $attachment_file );

			if ( is_wp_error( $metadata ) ) {

				if ( defined( 'WP_DEBUG' ) && WP_DEBUG )
					$this->log->add( 'csv-import', sprintf( __( '> > Error generating attachment metadata "%s": %s', 'wc_csv_import' ), $attachment_file, $metadata->get_error_message() ) );

			} elseif ( empty( $metadata ) ) {

				if ( defined( 'WP_DEBUG' ) && WP_DEBUG )
					$this->log->add( 'csv-import', sprintf( __( '> > Error generating attachment metadata "%s"', 'wc_csv_import' ), $attachment_file ) );
			
			} else {

				if ( defined( 'WP_DEBUG' ) && WP_DEBUG )
					$this->log->add( 'csv-import', sprintf( __( '> > Generated attachment metadata "%s"', 'wc_csv_import' ), $attachment_file ) );
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG )
				$this->log->add( 'csv-import', sprintf( __( '> > Updated attachment metadata "%s"', 'wc_csv_import' ), $attachment_file ) );
		}
	}
...
<?php
	...
	public function product_exists( $title, $sku = '' ) {
		global $wpdb;
