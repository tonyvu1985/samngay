...
class WC_PCSVIS_Product_Variation_Import extends WC_PCSVIS_Product_Import {
	...
	function process_product( $post ) {
		...
		if ( empty( $post['post_parent'] ) ) {
			...
			$this->log->add( 'csv-import', __('> Skipping - no post parent set.', 'wc_csv_import') );
			return;
		}
		...
		if ( ! empty( $post['post_id'] ) && isset( $this->processed_posts[$post['post_id']] ) ) {
			...
			$this->log->add( 'csv-import', __('> Post ID already processed. Skipping.', 'wc_csv_import') );
			return;
		}
		...
		if ( $post['post_status'] == 'auto-draft' ){
			...
			$this->log->add( 'csv-import', __('> Skipping auto-draft.', 'wc_csv_import') );
			return;
		}
		...
		if ($merging) {
			...
			if ( is_wp_error( $post_id ) ) {
				...
				$this->log->add( 'csv-import', sprintf( __( 'Failed to import product &#8220;%s&#8221;', 'wc_csv_import' ), esc_html($post['post_title']) ) );
				return;
		...
		if ( ! empty( $post['terms'] ) ) {
			...
			$this->log->add( 'csv-import', __( '> > Terms set', 'wc_csv_import' ) );
		...
		if ( ! empty( $post['postmeta'] ) ) {
			foreach ( $post['postmeta'] as $meta ) {
				...
				if ( $key ) {
					...
					$this->log->add( 'csv-import', sprintf( __('> > Updating custom field - %s.', 'wc_csv_import'), $key ) );
		...
		if ( ! empty( $post['images'] ) ) {
			...
			if ($merging) {
				...
				$this->log->add( 'csv-import', __( '> > Old images processed', 'wc_csv_import' ) );
		...
		if ($merging) {
			...
			$this->log->add( 'csv-import', sprintf( __('> Finished merging variation ID %s.', 'wc_csv_import'), $post_id ) );
		} else {
			$this->add_import_result( 'imported', 'Import successful', $post_id, get_the_title( $post_parent ), $post['sku'] );
			$this->log->add( 'csv-import', sprintf( __('> Finished importing variation ID %s.', 'wc_csv_import'), $post_id ) );
		}
	...
	function import_start( $file, $mapping, $start_pos, $end_pos ) {
		$this->log->add( 'csv-import', __( 'Parsing product variations CSV.', 'wc_csv_import' ) );
		...
		$this->log->add( 'csv-import', __( 'Finished parsing product variations CSV.', 'wc_csv_import' ) );
